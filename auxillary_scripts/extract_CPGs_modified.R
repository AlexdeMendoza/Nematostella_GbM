extract_CPGs_modified = function(ref_genome = NULL, seqnames) 
{
    ...
    if (is.null(ref_genome)) {
        ...
        if (nrow(gnoms_installed[pkgname %in% ref_genome]) == 
            0) {
    ...
    chrom_sizes = data.table::data.table(contig = seqnames, 
        length = seqlengths(x = ref_genome)[names(seqlengths(x = ref_genome)) %in% 
            seqnames])
    chrs = seqnames
    message("-Extracting CpGs")
    cgs = lapply(chrs, function(x) start(Biostrings::matchPattern("CG", 
        ref_genome[[x]])))
    cpgs = do.call(c, lapply(seq_along(chrs), function(x) GenomicRanges::GRanges(seqnames[x], 
    ...
    cpgs[, `:=`(chr, as.character(chr))][, `:=`(start, as.numeric(start))][, 
        `:=`(end, as.numeric(end))][, `:=`(width, as.numeric(width))]
    data.table::setkey(x = cpgs, "chr", "start")
    message(paste0("-Done. Extracted ", format(nrow(cpgs), big.mark = ","), 
        " CpGs from ", length(chrs), " contigs."))
