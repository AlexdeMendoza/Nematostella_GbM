# Nematostella GbM

Code associated to manuscript about Gene Body Methylation in *Nematostella vectensis*. If you use this code and/or data, please cite: 

Xu L, Heery R, Baranasic B, Žunar B, Segura Campaña A, Ovchinnikov V, Lenhard B, de Mendoza A. *Gene body methylation suppresses intragenic transcription and permits epigenetic inheritance in a cnidarian*. 2026. **Nature Ecology & Evolution**.

The repository is structured in 2 main folders

## Scripts

The scripts used to analyse different aspects of the manuscript, written in R or bash. 

## Data

Intermediate files obtained from the scripts, that are useful to reproduce the analysis. 

The raw data for this manuscript can be downloaded from GEO (GSE307384). 

